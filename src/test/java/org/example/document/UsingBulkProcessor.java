...
public class UsingBulkProcessor extends ElasticsearchClientBase {
    ...
    public void testBulkProcessor() throws Exception {
        ...
        bulkProcessor = BulkProcessor.builder(
                transportClient,  //增加elasticsearch客户端
                ...
                //设置自定义重复请求机制，最开始等待100毫秒，之后成倍更加，重试3次，当一次或多次重复请求失败后
                // 因为计算资源不够抛出 EsRejectedExecutionException 异常，可以通过BackoffPolicy.noBackoff()方法关闭重试机制
                .setBackoffPolicy(
                        BackoffPolicy.exponentialBackoff(TimeValue.timeValueMillis(100), 3))
                .build();

        //增加index
        bulkProcessor.add(new IndexRequest("twitter", "tweet", "8")
                .source(jsonBuilder()
                        .startObject()
                        .field("name", "bulkProcessor")
                        .endObject()));
        //删除
